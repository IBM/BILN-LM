...
def experiment(dataset: str, model: str, similarity_metric: str,
               fp: str, representation: str, pca: bool,
    ...
    for rep in representation.split('-'):
        x = np.concatenate([x, np.array(json.load(open(
            os.path.join('reps', f'{rep}_{dataset}.json'))))], axis=1)
    ...
    for th, partitions in hdg.get_partitions(filter=0.185):
        ...
        if pca:
            pca = dr.PCA(n_components=0.95, svd_solver='full')
            print(f'Number of dimensions before PCA: {train_x.shape[1]}')
            train_x = pca.fit_transform(train_x)
            print(f'Number of dimensions after PCA: {train_x.shape[1]}')
...
def main(dataset: str, model: str, similarity_metric: str,
         ...
         pca: Annotated[bool, typer.Option()] = False,
    ...
    results_path = os.path.join(
        results_dir,
        f'{dataset}_{model}_{similarity_metric}_{fp}_{representation}_{pca}.csv'
    )
    ...
    for i in tqdm(range(n_seeds)):
        ...
        result_df = experiment(
            ...
            fp, representation, pca, n_trials, i
        )
